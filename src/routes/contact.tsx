...
const channels: { icon: typeof Mail; label: string; value: string; note: string; href?: string }[] = [
  ...
  { icon: MessageCircle, label: "WhatsApp", value: "+91 99999 55555", note: "Tap to chat — reply in minutes", href: "https://wa.me/919999955555?text=Hi%20Sangam%2C%20I%20have%20a%20question." },
    ...
    <SiteLayout>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_1.2fr] lg:px-8">
        <div className="grid gap-4 self-start">
          {channels.map((c) => {
            const inner = (
              <div className="flex items-start gap-4 rounded-2xl border border-border bg-card p-5 shadow-soft transition hover:shadow-elegant">
            ...
            if ("href" in c && c.href) {
              return <a key={c.label} href={c.href} target="_blank" rel="noopener">{inner}</a>;
            }
